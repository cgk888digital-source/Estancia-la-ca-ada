...
const RestaurantMenu: React.FC<{ 
  ...
  return (
    ...
    >
      ...
      {/* ── HEADER CON VIDEO ── */}
      <div className="relative h-[28vh] min-h-[200px] max-h-[280px] w-full overflow-hidden">
        <video
          src="/assets/restaurante/video-cocina.mov"
          poster="/assets/restaurante/hero.jpg"
          autoPlay
          loop
          muted
          playsInline
        ...
        <button
          onClick={onBack}
          aria-label="Volver"
          className="absolute top-4 left-4 z-20 p-2 bg-black/40 backdrop-blur-md rounded-full border border-white/20 text-white shadow-md active:scale-95 transition-transform"
        >
          <ChevronLeft size={20} />
        </button>
        ...
        <div className="absolute bottom-0 left-0 w-full p-5">
          <motion.div initial={{ y: 16, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.15 }}>
            <span className="text-brand-accent text-[10px] uppercase tracking-[0.4em] font-bold block mb-1">
              Cocina de La Cañada
            </span>
            <h1 className="text-white text-2xl sm:text-3xl font-serif leading-tight">
      ...
      {/* ── MENÚ DE LA SEMANA (DIRECTO TRAS EL VIDEO) ── */}
      ...
      {/* ── AMBIENTE — chefs + interior (AL FINAL) ── */}
      <section className="px-5 mt-14">
        ...
        <motion.div
          ...
          style={{ height: 200 }}
        >
          <img src="/assets/restaurante/Comedor.png" alt="Restaurante La Cañada" className="w-full h-full object-cover" />
